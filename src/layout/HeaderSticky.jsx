        ...
        <div className='sticky top-0 z-20 w-full bg-rose-500 border-b-2 border-black text-white'>
            ...
            <div className={`${isOpen ? 'flex' : 'hidden'} md:flex flex-col md:flex-row w-full bg-rose-500`}>
                ...
                {categoryList.map((category, i) => (
                    ...
                    <div onClick={() => handleCategoryClick(category)} key={i} className='py-3 px-6 cursor-pointer hover:bg-rose-600 border-b border-rose-400 md:border-b-0 md:border-r md:border-rose-400 last:border-0 transition-colors text-center md:text-left text-sm'>{category}</div>
                ))}
